...
func getCurrencyFromRequest(request *http.Request) (float64, model.RequestError) {
	...
	currencyValue := tryGettingCurrencyValue(requestError, currencyName)
	return currencyValue, requestError
}

func tryGettingCurrencyValue(requestError models.RequestError, currencyName string) float64 {
